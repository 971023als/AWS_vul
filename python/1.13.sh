...
jsonData = {
    ...
    "대응방안": ("클라우드 환경에서는 모든 API 및 리소스 작업 시 익명 사용자의 접근을 비활성화해야 합니다. 쿠버네티스는 'system:anonymous'에 대한 권한을 부여할 수 있는 RoleBinding을 생성할 수 있으며, "
                 "이는 보안에 취약할 수 있습니다. Kubernetes/EKS 버전 1.14 이전에는 'system:unauthenticated' 그룹이 일부 기본 Cluster 역할에 연결되므로 업데이트 후에도 이 권한이 유지되지 않도록 주의해야 합니다."),
...
try:
    ...
    if 'system:anonymous' in access_check or 'system:unauthenticated' in access_check:
        result = "Unnecessary anonymous access permissions detected:\n" + access_check
        jsonData['진단결과'] = "취약"
        # Remove permissions
        # Here you would include logic to identify and delete specific bindings or roles
        print("Removing permissions...")
        # Example: subprocess.run(["kubectl", "delete", "clusterrolebinding", "binding-name"])
        print("Permissions removed.")
    else:
        result = "No unnecessary anonymous access permissions found.\n"
        jsonData['진단결과'] = "양호"
except subprocess.CalledProcessError as e:
    result = f"Failed to check or modify anonymous access permissions: {e}\n"
    jsonData['진단결과'] = "에러 발생"
